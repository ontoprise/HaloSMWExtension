...
class LdapAuthenticationPlugin extends AuthPlugin {
	...
	function connect() {
		...
		// KK: Patch for Apache Directory (LDAP) -----------
		// do not request with a URL, e.g. ldap://localhost
		if (isset($wgLDAPOptions['no_url'])) {
			$serverpre = "";
		}
		// -------------------------------------------------
		...
		$ldapconn = @ldap_connect( $servers , isset($wgLDAPOptions) ? $wgLDAPOptions['port'] : 389);
	...
	function getGroups( $ldapconn, $dn ) {
		global $wgLDAPGroupObjectclass, $wgLDAPGroupAttribute, $wgLDAPGroupNameAttribute, $wgLDAPGroupAttributeValue;
		global $wgLDAPProxyAgent, $wgLDAPProxyAgentPassword;
		global $wgUser;
        ...
        // KK: Configurable $searchvalue patch ---------------------------- 
        $searchValue = $wgLDAPGroupAttributeValue[$_SESSION['wsDomain']];
        $searchValue = str_replace("USER-NAME", $value, $searchValue);
		$filter = "(&($attribute=$searchValue)(objectclass=$objectclass))";
		// ----------------------------------------------------------------
