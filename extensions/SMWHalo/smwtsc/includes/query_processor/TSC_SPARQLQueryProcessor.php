		...
		foreach ($rawparams as $name => $param) {
			...
			if (count($parts) > 1) { // label found, use this instead of default
				...
				if (TSHelper::isSPARQL($param)) {
					...
					$parts = explode('=',$param,2);
					$knownOption = in_array($parts[0], array('merge','template', 'mainlabel', 'sort', 'order', 'default', 'format', 'offset', 'limit', 'headers', 'link', 'intro', 'searchlabel', 'enable add', 'enable delete', 'use silent annotations template', 'write protected annotations', 'instance name preload value'));
