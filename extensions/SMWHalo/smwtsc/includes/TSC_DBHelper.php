...
class TSCDBHelper {
    ...
    public static function setupTable($table, $fields, $db, $verbose, $primaryKeys = "") {
        global $wgDBname;
        TSCDBHelper::reportProgress("Setting up table $table ...\n",$verbose);
        if ($db->tableExists($table) === false) { // create new table
            ...
            $db->query( $sql, 'TSCDBHelper::setupTable' );
            TSCDBHelper::reportProgress("   ... new table created\n",$verbose);
            return array();
        } else { // check table signature
            TSCDBHelper::reportProgress("   ... table exists already, checking structure ...\n",$verbose);
            $res = $db->query( 'DESCRIBE ' . $table, 'TSCDBHelper::setupTable' );
            $curfields = array();
            $result = array();
            ...
            foreach ($fields as $name => $type) {
                if ( !array_key_exists($name,$curfields) ) {
                    TSCDBHelper::reportProgress("   ... creating column $name ... ",$verbose);
                    $db->query("ALTER TABLE $table ADD `$name` $type $position", 'TSCDBHelper::setupTable');
                    $result[$name] = 'new';
                    ...
                    TSCDBHelper::reportProgress("   ... changing type of column $name from '$curfields[$name]' to '$type' ... ",$verbose);
                    $db->query("ALTER TABLE $table CHANGE `$name` `$name` $type $position", 'TSCDBHelper::setupTable');
                    $result[$name] = 'up';
                    $curfields[$name] = false;
                    TSCDBHelper::reportProgress("done.\n",$verbose);
                } else {
                    TSCDBHelper::reportProgress("   ... column $name is fine\n",$verbose);
                    $curfields[$name] = false;
                }
                $position = "AFTER $name";
            }
            foreach ($curfields as $name => $value) {
                if ($value !== false) { // not encountered yet --> delete
                    TSCDBHelper::reportProgress("   ... deleting obsolete column $name ... ",$verbose);
                    $db->query("ALTER TABLE $table DROP COLUMN `$name`", 'TSCDBHelper::setupTable');
                    $result[$name] = 'del';
                    TSCDBHelper::reportProgress("done.\n",$verbose);
                }
            }
            TSCDBHelper::reportProgress("   ... table $table set up successfully.\n",$verbose);
            return $result;
        }
    }
    ...
    public static function getSQLOptionsAsString($requestoptions, $valuecol = NULL) {
        $options = TSCDBHelper::getSQLOptions($requestoptions,$valuecol);
