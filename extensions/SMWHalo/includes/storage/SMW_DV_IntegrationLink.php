...
class SMWURIIntegrationValue extends SMWDataValue {
    ...
    public function SMWURIIntegrationValue( $typeid ) {
...
//                  }
                ...
                case SMW_URI_MODE_TEL:
                    ...
                    if ( ( strlen( preg_replace( '/[^0-9]/', '', $value ) ) < 6 ) ||
                         ( preg_match( '<[-+./][-./]>', $value ) ) ||
                         ( !SMWURIIntegrationValue::isValidTelURI( $value ) ) ) { ///TODO: introduce error-message for "bad" phone number
    ...
    public function getShortWikiText( $linked = null ) {
        ...
        if ( ( $linked === null ) || ( $linked === false ) || ( $this->m_outformat == '-' ) || ( $this->m_url == '' ) || ( $this->m_caption == '' ) ) {
        	...
        	return '<ilink caption="'.htmlspecialchars($this->m_caption).'">'.$this->m_url.'</ilink>';
        }
    }
