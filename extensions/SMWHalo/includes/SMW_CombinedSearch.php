 ...
 */
 	...
 	// try to find $parts as attribute values
 		...
 		if (count($titlesAndValues) > 0) { 
 			...
 			foreach($titlesAndValues as $tav) {
 				...
 				$htmlResult .= "<tr><td><a href=\"$wgServer$wgScriptPath/index.php/".$instNSWithColon.$title->getText()."\">".$title->getText()."</a></td>" .
 								   "<td><a href=\"$wgServer$wgScriptPath/index.php/".$attrNSWithCol.$attribute->getText()."\">".$attribute->getText()."</a></td>" .
 								   "<td>$value</td></tr>";
 			}
 			$htmlResult .= "</table>";
 		} else {
 ...
 class CombinedSearch {
 	...
 	private function getPage($entityTitle) {
		...
		} else if (smwfGetStore()->supportsFunction('editdistance')) { 
