...
class SMWSemanticStoreSQL2 extends SMWSemanticStoreSQL {
    ...
    protected function createVirtualTableWithPropertiesByName($requestoptions, & $db) {
        ...
        $db->query('INSERT INTO smw_ob_properties (SELECT smw_id, smw_title FROM '.$smw_ids.' WHERE smw_iw != ":smw-redi" AND smw_namespace = '.SMW_NS_PROPERTY.' '. $sql.')'); 
        $sql = DBHelper::getSQLConditions($requestoptions,'s.smw_title','s.smw_title');
        // add targets of matching redirects
        $db->query('INSERT INTO smw_ob_properties (SELECT s2.smw_id, s2.smw_title FROM '.$smw_ids.' s JOIN '.$smw_ids.' s2 ON s.smw_sortkey = s2.smw_title WHERE s.smw_namespace = '.SMW_NS_PROPERTY.' AND s.smw_iw=":smw-redi"'. $sql.')');             
    }
