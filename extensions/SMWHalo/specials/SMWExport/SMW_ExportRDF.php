...
function getPageAsRDF($page = '') {
	...
	if( isset($page) ) {  // export to RDF
		...
		$exRDF = new ExportRDFHalo();
		if ('' !== $date) $exRDF->setDate($date);
		return $exRDF->getPage($page,$recursive,$backlinks);
	}
}
...
function doSpecialExportRDF($page = '') {
	...
	if ($wgRequest->getVal('exportAll') == 'Export All') {
		$exRDF = new ExportRDFHalo();
		$date = $wgRequest->getText( 'date' );
		if ('' == $date) $date = $wgRequest->getVal( 'date' );
		if ('' !== $date) $exRDF->setDate($date);
		$exRDF->printAll("allRDFData.owl");
		$wgOut->addHTML("All semantic data was exported to: allRDFData.owl");
	}
	...
	if ( $wgUser->isAllowed('delete') || $smwgExportAll) {
		$html .= '<input type="checkbox" name="exportAll" value="Export All" default="true" id="ea">&nbsp;<label for="ea">' . wfMsg('smw_exportrdf_all') . '</label></input><br />' . "\n";
	}
	if ( $wgUser->isAllowed('delete') || $smwgExportAll) {
		$html .= '<br />';
...
class ExportRDFHalo extends ExportRDF {
	...
	/**
	 * See documentation for ExportRDFHalo::pre_ns_buffer.
	 */
	var $post_ns_buffer;
	...
	public function printPages($pages, $recursion = 1, $backlinks = true) {
		...
		foreach ($pages as $page) {
			...
			$et = new SMWExportTitleHalo($title, $this);
	...
	public function getPage($page, $recursion = 1, $backlinks = true) {
		$this->store = &smwfGetStore();
		$this->pre_ns_buffer = '';
		$this->post_ns_buffer = '';
		$this->first_flush = true;
...
//			print "\nWriting RDF dump to file \"$outfile\" ...\n";
		...
		for ($id = $start; $id <= $end; $id++) {
			...
			if ( !ExportRDFHalo::fitsNsRestriction($ns_restriction, $title->getNamespace()) ) continue;
			$cur_queue = array(new SMWExportTitleHalo($title, $this));
			$a_count++; //DEBUG
			$full_export = true;
			while (count($cur_queue) > 0) {
				foreach ( $cur_queue as $et) {
				...
				foreach ($this->element_queue as $key => $etq) {
					if ( (!$etq->exists) || !smwfIsSemanticsProcessed($etq->title_namespace) ||
					      !ExportRDFHalo::fitsNsRestriction($ns_restriction, $etq->title_namespace) ||
					      $etq->modifier !== '') {
	 ...
	 *
	 * @param SMWExportTitleHalo $et The Exporttitle wrapping the page to be exported
...
// 			}
	...
	/** Fetch SMWExportTitleHalo for a given article. The inputs are
	...
	/**
	 * Fetch SMWExportTitleHalo for a given article that is identified by its
	 * article id.
	 */
	...
	/**
	 * Fetch SMWExportTitleHalo for a given article. Caches are used to
	...
	private function getExportTitleFromTitle($title, $modifier = '') {
		...
		if (array_key_exists($key, $this->element_done)) {
			...
			$result = new SMWExportTitleHalo($title, $this, $modifier);
	...
	private function markAsDone($et) {
		if ( count($this->element_done) >= ExportRDFHalo::MAX_CACHE_SIZE ) {
			$this->element_done = array_slice( $this->element_done,
										ExportRDFHalo::CACHE_BACKJUMP,
										ExportRDFHalo::MAX_CACHE_SIZE - ExportRDFHalo::CACHE_BACKJUMP,
										true );
		}
