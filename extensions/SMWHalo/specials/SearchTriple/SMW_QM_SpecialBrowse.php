...
/**
* This is a patched version of SMWSpecialBrowse which is shipped 
* together wuth Semantic MediaWiki.
* 
*  Modifications: The method displayData is modified. The property 
*  QRCUsesQueryCall is filtered out from displayed properties.
 */
 ...
 * @ingroup SpecialPage
 */
class SMWQMSpecialBrowse extends SpecialPage {
	...
	public function __construct() {
		...
		if ( $smwgBrowseShowAll ) {
			SMWQMSpecialBrowse::$incomingvaluescount = 21;
			SMWQMSpecialBrowse::$incomingpropertiescount = - 1;
		}
	}
	...
	private function displayData( SMWSemanticData $data, $left = true, $incoming = false ) {
		...
		foreach ( $properties as $property ) {
			$displayline = true;
			if($property->getWikiValue() == QRC_UQC_LABEL){
				$displayline = false;
			...
			if ( $displayline ) {
				...
				$count = count( $values );
				$more = ( $count >= SMWQMSpecialBrowse::$incomingvaluescount );
	...
	private function displayBottom( $more ) {
		...
		if ( !$smwgBrowseShowAll ) {
			if ( ( $this->offset > 0 ) || $more ) {
				$offset = max( $this->offset - SMWQMSpecialBrowse::$incomingpropertiescount + 1, 0 );
				$html .= ( $this->offset == 0 ) ? wfMsg( 'smw_result_prev' ):
					     $this->linkhere( wfMsg( 'smw_result_prev' ), $this->showoutgoing, true, $offset );
				$offset = $this->offset + SMWQMSpecialBrowse::$incomingpropertiescount - 1;
