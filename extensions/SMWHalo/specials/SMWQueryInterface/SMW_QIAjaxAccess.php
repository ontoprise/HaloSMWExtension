...
function smwf_qi_getAskPage($args= "") {
	...
	if (array_key_exists('returnObject', $params)) {
		$returnObject = $params['returnObject'];
	} else {
		$returnObject = 'smwhgASKQuery';
	}
	...
	$html = str_replace('</body>', '<input id="stb-qi-save-button" type="button" value="OK" name="ok" onclick="window.parent.'.$returnObject.'.saveQuery();" /></body>', $html);
	$html = str_replace('</body>', '<input id="stb-qi-cancel-button" type="button" value="Cancel" name="cancel" onclick="window.parent.'.$returnObject.'.cancelQuery();" /></body>', $html);
