...
******/
	...
	function EditAreaLoader(){
		...
		this.sub_scripts_to_load= new Array("edit_area", "manage_area" ,"edit_area_functions", "keyboard", "highlight", "regexp");
	...
	EditAreaLoader.prototype.start= function(id){
		...
		if(!this.iframe_css){
			if(this.nav['isFirefox']){
				this.iframe_css="<link href='"+ this.baseURL +"edit_area_ff.css' rel='stylesheet' type='text/css' />";
			}
			else {
		...
		template= template.replace("[__WRAP__]", wrap);
	...
	*/
	...
	/*
	* Whenever the USER toggles the editor, set the cookie if it is enabled
	* or not. You can not use the standard toggle function, since it is also
	* called whenever a user leaves the page.
	*/
	EditAreaLoader.prototype.userToggle= function(id, toggle_to){
	...
	EditAreaLoader.prototype.setCookie = function(state) {

		var a = new Date();
		a = new Date(a.getTime() +1000*60*60*24*365);

		document.cookie = 'smwUseAdvancedEditor='+state+'; expires='+a.toGMTString()+';';
	};
	...
	EditAreaLoader.prototype.toggle_on= function(id){
		...
		// register textarea for auto-completion
    ...
    EditAreaLoader.prototype.insertTags = function(id, open_tag, close_tag, example_text){
    	var old_sel= this.getSelectionRange(id);
    	if (this.getSelectedText(id) != ""){
    		text= open_tag + this.getSelectedText(id) + close_tag;
    	} else {
    		text = open_tag + example_text + close_tag;
    	}
    	...
    	if(old_sel["end"] > old_sel["start"])	// if text was selected, cursor at the end
    		...
    		this.setSelectionRange(id, old_sel["start"]+open_tag.length, old_sel["start"]+open_tag.length+example_text.length);
    };
