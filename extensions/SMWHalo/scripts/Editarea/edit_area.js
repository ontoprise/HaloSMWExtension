...
******/
	...
	EditArea.prototype.init= function(){
		...
		// init events
		document.getElementById("textarea").onfocus= onFocus;
		document.getElementById("textarea").onblur= onBlur;

		document.getElementById("editor").onkeyup= keyUp;
		...
		/*	if(this.last_selection["nb_line"] >= this.line_number)
			...
			{	// need height resizing
				...
				if(this.nav["isFirefox"]){
					var newheight = this.getAdditionalLines();
					this.container.style.height= (newheight+2)+"px";
					this.textarea.style.height= newheight+"px";
					this.content_highlight.style.height= newheight+"px";
					this.textarea.previous_scrollHeight= newheight;
				}
				else{
	...
	EditArea.prototype.getAdditionalLines = function(){
		var avglines = Math.ceil(this.textarea.value.length/90);
		var lines = this.textarea.value.split("\n");
		return this.lineHeight * (avglines+lines.length);
	};
