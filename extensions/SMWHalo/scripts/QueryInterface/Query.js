...
*/
    updateTree:function(){
        ...
        // tree in load query tab doesn't need links
        var nodeLink = ($('qiMainTab1').className == 'qiDefTabActive');
		...
		for(var i=0; i<this.properties.length; i++){
            ...
            if (nodeLink)
                tree += '<a href="javascript:void(0);" onclick="qihelper.selectNode(this, \'property-'+this.id+'-'+i+'\')">';
            tree += this.properties[i].getName();
            if (nodeLink) tree += '</a>'
            tree += '</td></tr>'
		...
		for(var i=0; i<this.properties.length; i++){
			...
			if(this.properties[i].getArity() > 2){ // always special treatment for arity > 2
				...
				for(var j=0; j<vals.length; j++){
                    ...
                    if (vals[j][1] != "=") //add operator <, >, ! etc. if existing
                        tmpAsk += vals[j][1];
                    // value set?
                    if (vals[j][2] != '*') {
