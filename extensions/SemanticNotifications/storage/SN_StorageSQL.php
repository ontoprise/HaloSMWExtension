...
class SNStorageSQL {
	...
	/**
	 * Deletes all database tables of SemanticNotifications
	 * @param bool $verbose
	 */
	public function dropDatabaseTables($verbose = true) {
		global $wgDBtype;
		
		SGADBHelper::reportProgress("Deleting all database content and tables generated by SemanticNotifications ...\n\n",$verbose);
		$db =& wfGetDB( DB_MASTER );
		$tables = array(
			'smw_sem_notification');
		foreach ($tables as $table) {
			$name = $db->tableName($table);
			$db->query('DROP TABLE' . ($wgDBtype=='postgres'?'':' IF EXISTS'). $name, 'SNStorageSQL::dropDatabaseTables');
