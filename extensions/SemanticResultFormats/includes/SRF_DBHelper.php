...
class SRFDBHelper {
	...
	public static function setupTable($table, $fields, $db, $verbose, $primaryKeys = "") {
		global $wgDBname;
		SRFDBHelper::reportProgress("Setting up table $table ...\n",$verbose);
		if ($db->tableExists($table) === false) { // create new table
			...
			SRFDBHelper::reportProgress("   ... table exists already, checking structure ...\n",$verbose);
			$res = $db->query( 'DESCRIBE ' . $table, 'SRFDBHelper::setupTable' );
			$curfields = array();
			$result = array();
			...
			foreach ($fields as $name => $type) {
				if ( !array_key_exists($name,$curfields) ) {
					SRFDBHelper::reportProgress("   ... creating column $name ... ",$verbose);
					$db->query("ALTER TABLE $table ADD `$name` $type $position", 'SRFDBHelper::setupTable');
					$result[$name] = 'new';
					SRFDBHelper::reportProgress("done \n",$verbose);
				} elseif ($curfields[$name] != $type) {// && stripos("auto_increment", $type) == -1) {
					SRFDBHelper::reportProgress("   ... changing type of column $name from '$curfields[$name]' to '$type' ... ",$verbose);
					$db->query("ALTER TABLE $table CHANGE `$name` `$name` $type $position", 'SRFDBHelper::setupTable');
					$result[$name] = 'up';
					$curfields[$name] = false;
					SRFDBHelper::reportProgress("done.\n",$verbose);
				} else {
					SRFDBHelper::reportProgress("   ... column $name is fine\n",$verbose);
					$curfields[$name] = false;
				}
				$position = "AFTER $name";
			}
			foreach ($curfields as $name => $value) {
				if ($value !== false) { // not encountered yet --> delete
					SRFDBHelper::reportProgress("   ... deleting obsolete column $name ... ",$verbose);
					$db->query("ALTER TABLE $table DROP COLUMN `$name`", 'SRFDBHelper::setupTable');
					$result[$name] = 'del';
					SRFDBHelper::reportProgress("done.\n",$verbose);
				}
			}
			SRFDBHelper::reportProgress("   ... table $table set up successfully.\n",$verbose);
			return $result;
		}
	}
	...
	public static function setupIndex($table, $columns, $db) {
		...
		$res = $db->query( 'SHOW INDEX FROM ' . $table , 'SRF::SetupIndex');
		if ( !$res ) {
			return false;
		}
		$indexes = array();
		...
		foreach ($indexes as $key => $index) { // clean up existing indexes
			...
			if ( $id !== false ) {
				...
				if($key != 'PRIMARY') {
					$db->query( 'DROP INDEX ' . $key . ' ON ' . $table, 'SRF::SetupIndex');
				}
			}
		}
		...
		foreach ($columns as $column) { // add remaining indexes
			if ($column != false) {
				$db->query( "ALTER TABLE $table ADD INDEX ( $column )", 'SRF::SetupIndex');
			}
		}
		return true;
	}
	...
	public static function getSQLOptionsAsString($requestoptions, $valuecol = NULL) {
		$options = SRFDBHelper::getSQLOptions($requestoptions,$valuecol);
