if(typeof(ofc_data_objs)=="undefined") ofc_data_objs = {data:[],tabs:[],showhide:[]};
document.ofc = { js:{} };
(function($) {
 //$.noConflict(); EVIL!!!
	...
	$(document).ready(function(){
		...
		for(var i=0;i<ofc_data_objs.tabs.length;++i) {
			$(ofc_data_objs.tabs[i]).click(document.ofc.js.tabChart);
		}
		for(var i=0;i<ofc_data_objs.showhide.length;++i) {
			$(ofc_data_objs.showhide[i]).click(document.ofc.js.showHideChart);
		}
		$("a.ofc_table_link").click(function(event){
		  	event.preventDefault();
		    var the_id=$(this).attr("id").substr(16);
