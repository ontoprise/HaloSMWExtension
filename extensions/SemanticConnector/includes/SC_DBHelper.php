...
class SCDBHelper {
    ...
    public static function setupTable($table, $fields, $db, $verbose, $primaryKeys = "") {
        global $wgDBname;
        SCDBHelper::reportProgress("Setting up table $table ...\n",$verbose);
        if ($db->tableExists($table) === false) { // create new table
            ...
            SCDBHelper::reportProgress("   ... table exists already, checking structure ...\n",$verbose);
            $res = $db->query( 'DESCRIBE ' . $table, 'SCDBHelper::setupTable' );
            $curfields = array();
            $result = array();
            ...
            foreach ($fields as $name => $type) {
                if ( !array_key_exists($name,$curfields) ) {
                    SCDBHelper::reportProgress("   ... creating column $name ... ",$verbose);
                    $db->query("ALTER TABLE $table ADD `$name` $type $position", 'SCDBHelper::setupTable');
                    $result[$name] = 'new';
                    SCDBHelper::reportProgress("done \n",$verbose);
                } elseif ($curfields[$name] != $type) {// && stripos("auto_increment", $type) == -1) {
                    SCDBHelper::reportProgress("   ... changing type of column $name from '$curfields[$name]' to '$type' ... ",$verbose);
                    $db->query("ALTER TABLE $table CHANGE `$name` `$name` $type $position", 'SCDBHelper::setupTable');
                    $result[$name] = 'up';
                    $curfields[$name] = false;
                    SCDBHelper::reportProgress("done.\n",$verbose);
                } else {
                    SCDBHelper::reportProgress("   ... column $name is fine\n",$verbose);
                    $curfields[$name] = false;
                }
                $position = "AFTER $name";
            }
            foreach ($curfields as $name => $value) {
                if ($value !== false) { // not encountered yet --> delete
                    SCDBHelper::reportProgress("   ... deleting obsolete column $name ... ",$verbose);
                    $db->query("ALTER TABLE $table DROP COLUMN `$name`", 'SCDBHelper::setupTable');
                    $result[$name] = 'del';
                    SCDBHelper::reportProgress("done.\n",$verbose);
                }
            }
            SCDBHelper::reportProgress("   ... table $table set up successfully.\n",$verbose);
            return $result;
        }
    }
    ...
    public static function getSQLOptionsAsString($requestoptions, $valuecol = NULL) {
        $options = SCDBHelper::getSQLOptions($requestoptions,$valuecol);
