...
class NewUserMessage {
	...
	static function createNewUserMessage( $user ) {
		...
		if ( !$userpage->exists() ) {
			...
			if ( $templateTitle->getNamespace() == NS_TEMPLATE ) {
				$templateForUserpage = new Article($templateTitle);
			}
			...
			$text = $templateForUserpage->getContent();
    ...
    static function onNewUserRedirect( $user ) {
		global $wgOut;

		$wgOut->redirect( $user->getUserPage()->getFullUrl()."?action=formedit" );
