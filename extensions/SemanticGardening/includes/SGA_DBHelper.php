...
class SGADBHelper {
    ...
    public static function setupTable($table, $fields, $db, $verbose, $primaryKeys = "") {
        global $wgDBname;
        SGADBHelper::reportProgress("Setting up table $table ...\n",$verbose);
        if ($db->tableExists($table) === false) { // create new table
            ...
            SGADBHelper::reportProgress("   ... table exists already, checking structure ...\n",$verbose);
            $res = $db->query( 'DESCRIBE ' . $table, 'SGADBHelper::setupTable' );
            $curfields = array();
            $result = array();
            ...
            foreach ($fields as $name => $type) {
                if ( !array_key_exists($name,$curfields) ) {
                    SGADBHelper::reportProgress("   ... creating column $name ... ",$verbose);
                    $db->query("ALTER TABLE $table ADD `$name` $type $position", 'SGADBHelper::setupTable');
                    $result[$name] = 'new';
                    SGADBHelper::reportProgress("done \n",$verbose);
                } elseif ($curfields[$name] != $type && stripos("auto_increment", $type) == -1) {
                    SGADBHelper::reportProgress("   ... changing type of column $name from '$curfields[$name]' to '$type' ... ",$verbose);
                    $db->query("ALTER TABLE $table CHANGE `$name` `$name` $type $position", 'SGADBHelper::setupTable');
                    $result[$name] = 'up';
                    $curfields[$name] = false;
                    SGADBHelper::reportProgress("done.\n",$verbose);
                } else {
                    SGADBHelper::reportProgress("   ... column $name is fine\n",$verbose);
                    $curfields[$name] = false;
                }
                $position = "AFTER $name";
            }
            foreach ($curfields as $name => $value) {
                if ($value !== false) { // not encountered yet --> delete
                    SGADBHelper::reportProgress("   ... deleting obsolete column $name ... ",$verbose);
                    $db->query("ALTER TABLE $table DROP COLUMN `$name`", 'SGADBHelper::setupTable');
                    $result[$name] = 'del';
                    SGADBHelper::reportProgress("done.\n",$verbose);
                }
            }
            SGADBHelper::reportProgress("   ... table $table set up successfully.\n",$verbose);
            return $result;
        }
    }
    ...
    public static function getSQLOptionsAsString($requestoptions, $valuecol = NULL) {
        $options = SGADBHelper::getSQLOptions($requestoptions,$valuecol);
