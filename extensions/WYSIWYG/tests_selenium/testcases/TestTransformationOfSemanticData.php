...
class TestTransformationOfSemanticData extends SeleniumTestCase_Base
{
  ...
  public function testMyTestCase()
  {
    ...
    for ($second = 0; ; $second++) {
        if ($second >= 60) $this->fail("timeout");
        try {
            if ($this->isTextPresent("Helium is the chemical element with atomic number 2, which is represented by the symbol He.It is a <span class=\"fck_mw_property\" property=\"has color::none\">colorless</span>, <span class=\"fck_mw_property\" property=\"smells like\">odorless</span>, tasteless, non-toxic, inert monatomic gas that heads the noble gas group in the periodic table.")) break;
        } catch (Exception $e) {}
        sleep(1);
    }

    try {
        $this->assertTrue($this->isTextPresent("Helium is the chemical element with atomic number 2, which is represented by the symbol He.It is a <span class=\"fck_mw_property\" property=\"has color::none\">colorless</span>, <span class=\"fck_mw_property\" property=\"smells like\">odorless</span>, tasteless, non-toxic, inert monatomic gas that heads the noble gas group in the periodic table."), "Text not present: Helium is the chemical element with atomic number 2, which is represented by the symbol He.It is a <span class=\"fck_mw_property\" property=\"has color::none\">colorless</span>, <span class=\"fck_mw_property\" property=\"smells like\">odorless</span>, tasteless, non-toxic, inert monatomic gas that heads the noble gas group in the periodic table.");
    } catch (PHPUnit_Framework_AssertionFailedError $e) {
        array_push($this->verificationErrors, $e->toString());
    }
  }
