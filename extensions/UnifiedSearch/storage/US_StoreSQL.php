...
class USStoreSQL extends USStore {
	...
	public function lookUpTitlesByText($terms, array $namespaces, $disjunctive = false, $limit=10, $offset=0) {
		...
		$totalNum = 0;
		$query = '(SELECT COUNT(*) AS num FROM '.$page.' WHERE '.$sql.' AND page_is_redirect = 0) ' .
                            'UNION DISTINCT ' .
                              '(SELECT COUNT(*) AS num FROM '.$page.' JOIN redirect ON page_id = rd_from WHERE '.$sql.' AND page_is_redirect = 1)';
        $res = $db->query($query );
	    if($db->numRows( $res ) > 0) {
            while($row = $db->fetchObject($res)) {
                $totalNum += $row->num;
