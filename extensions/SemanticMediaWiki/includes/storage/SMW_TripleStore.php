...
require_once( "$smwgIP/includes/storage/stompclient/Stomp.php" );
...
class SMWTripleStore extends SMWStore {
	 ...
	 */
	function __construct(SMWStore $smwstore) {
		$this->smwstore = $smwstore;
	...
	function getSpecialValues(Title $subject, $specialprop, $requestoptions = NULL) {
		return $this->smwstore->getSpecialValues($subject, $specialprop, $requestoptions);
	}

	function getSpecialSubjects($specialprop, SMWDataValue $value, $requestoptions = NULL) {
		return $this->smwstore->getSpecialSubjects($specialprop, $value, $requestoptions);
	}
	...
	function updateData(SMWSemanticData $data, $newpage) {
		$this->smwstore->updateData($data, $newpage);
		...
		foreach($data->getProperties() as $key => $property) {
			...
			if ($triplesFromHook === false || count($triplesFromHook) > 0) {
				...
				continue; // do not process normal tripel generation, if hook provides triples.
			}
			if ($property instanceof Title) { // normal property
				foreach($propertyValueArray as $value) {
					if ($value->isValid() && !$value->isDerived()) {
						...
						if ($value->getTypeID() == '_txt') {
							$triples[] = array($subj_ns.":".$subject->getDBkey(), "prop:".$property->getDBkey(), "\"".$this->escapeQuotes($value->getXSDValue())."\"");
							 
						} elseif ($value->getTypeID() == '_wpg') {
							$obj_ns = $this->getNSPrefix($value->getNamespace());
							if ($obj_ns == NULL) continue;
							$triples[] = array($subj_ns.":".$subject->getDBkey(), "prop:".$property->getDBkey(), $obj_ns.":".$value->getDBkey());
							...
							if ($value->getUnit() != '') {
								$triples[] = array($subj_ns.":".$subject->getDBkey(), "prop:".$property->getDBkey(), "\"unit_(".$value->getXSDValue().",".$value->getUnit().")\"");
							} else {
								if ($value->getXSDValue() != NULL) {
									$triples[] = array($subj_ns.":".$subject->getDBkey(), "prop:".$property->getDBkey(), "\"".$this->escapeQuotes($value->getXSDValue())."\"");
								} else if ($value->getNumericValue() != NULL) {
									$triples[] = array($subj_ns.":".$subject->getDBkey(), "prop:".$property->getDBkey(), "\"".$value->getNumericValue()."\"");
								}
							}
				...
				switch ($property) {
					...
					default: // normal special value: do nothing
						continue;
