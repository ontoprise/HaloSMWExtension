...
$wikipediaText = urlencode(file_get_contents("wptext.txt"));
$ultrapediaText = urlencode(file_get_contents("uptext.txt"));
...
$cc = new cURL();
$loginXML = $cc->post($url."api.php","action=login&lgname=WikiSysop&lgpassword=root&format=xml");
echo $loginXML;
...
$param = "action=edit&title=Talk:Main_Page&summary=Hello%20World&text=".$ultrapediaText."&token=$et";
$editArticle = $cc->post($url."api.php", $param);

$param = "action=edit&title=Talk:Main_Page&summary=Hello%20World&text=$wikipediaText&token=$et";
$editArticle = $cc->post($url."api.php", $param);
