...
class SMW_DILanguageEn extends SMW_DILanguage {
	...
	'smw_wws_s4-help-triplification' => '<p>Please click the <b>Enable triplification</b> button and enter a <b>subject creation pattern</b> if you want to enable users to triplify the results of this web service. Triplifying web service results works like follows: A web service result always has a table like structure with rows and columns. A triple subject will be computed for each row via the Subject Creation Pattern. One triple then will be stored into the Triple Store for each cell of a row. The result part alias that corresponds to the table cell will be used as predicate and the cell value will be used as object. Names of properties which are already defined in the Wiki ontology should be used as result part aliases for two reasons. First, since they will be used as predicates. Second, because the Wiki will check for each result part alias, if there is a property with the same name and if that property has type information annotations and then will use the type information when creating triples. Alias input fields therefore autocomple on property names. The Subject Creation Pattern which will be used for computing triple subjects, can contain arbitrary Wiki text. You can use the syntax <i>?result.alias-name?</i> if you want to include cell values into the subject creation. The <i>?result.alias-name?</i> construct will then be replaced by the corresponding cell-value when subjects are created. Instead of typing this syntax on your own into the subject creation pattern input field, you can also click on an alias input field and it will automatically be added to the subject creation pattern.</p>',
