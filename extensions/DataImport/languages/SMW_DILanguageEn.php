...
class SMW_DILanguageEn extends SMW_DILanguage {
	...
	'smw_wws_s4-LD-help' => '<h4>Help</h4><p> Linked Data sources are encoded in one of the RDF serializations, e.g. RDF/XML or RDFa. Special result parts, i.e. one for extracting all subjects, one for extracting all predicates and one for extracting all objects, which occur in one of the triples, are generated by the system by default and you will later be able to use them, when calling the web service in an article via the allSubjects, allPredicates or allObjects aliases. You can click the <b>Add result parts</b> button, if you want to define further result parts for special predicates. Define in the table which further parts of this result users should be able to display in Wiki articles. Enter an <b>alias</b> for each result part. This alias will later be used to address the result part when the web service is used in an article. If you do not specify an alias, then a random one will be created. </p> Enter the URI of the predicate which should be extracted.</p><p><p>Press the <b>Add namespace prefixes</b> button if you want to define namespace prefixes. A table will be shown where you can define namespace prefixes, which you then can use when defining result parts.</p>',
