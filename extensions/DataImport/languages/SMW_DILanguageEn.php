...
class SMW_DILanguageEn extends SMW_DILanguage {
	...
	'smw_wws_s3-help' => '<h4>Help</h4><p>Each parameter is identified by a <b>path</b>, which addresses it in the parameter type hierarchy and an <b>alias</b>, which displaces the path of the parameter. Alias are necessary to use parameters later in an easy way. Browse the parameters by expanding and collapsing the type hierarchy in the tree view. Web services with a plain hierarchy don’t have a tree view. </p><p>In some cases it is not necessary to pass a value for each parameter when calling a web service. If you do not need a parameter, leave it empty or mark it as optional. Activate the <b>‘Use’-Checkboxes</b>, to include the parameters into the WWSD. Thus users are later able to provide values for parameters when calling the web service. </p><p>You can use the <b>“Auto generate alias”</b> function <img src="$1"/> to generate an alias based on the selected parameter.If a parameter is not required for the web service, you can mark this parameter as optional. Mark a parameter as <b>optional</b>, if users call the web service without passing a value for this parameter. Otherwise users will get an error message. </p><p>Define a <b>default value</b> for a parameter, this value will be used if a parameter is not optional and if no value for that parameter was passed to the web service call. Define <b>subparameters</b> if necessary, e. g. to make using complex parameters easier to use.</p>',
