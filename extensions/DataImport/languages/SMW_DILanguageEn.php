...
class SMW_DILanguageEn extends SMW_DILanguage {
	...
	'smw_wws_s4-REST-help' => '<h4>Help</h4><br/>RESTful web services return a string as their result. This string can be a simple text or it can contain information encoded in a format like JSON or XML. You can define the <b>complete result</b> of the web service as result part by activating the corresponding use-checkbox. If you like to define further result parts, then you have to click on the <b>Add result parts</b> button, so that the table for defining them will be displayed. Here you can define which further parts of this result users can display in Wiki articles. At first you have to enter an <b>alias</b> for each result part. This alias will later be used to address the result part when the web service is used in an article. If you do not specify an alias, then a random one will be created. Now you have to define if the path you like to use for extracting result parts has the JSON or the XPath <b>format</b>. (Note: The Data import extension supports only the XPath format at the moment.) Now you can enter the <b>path</b> which will be used to extract the result part from the complete result.',
	...
	'smw_wws_yourws' => 'Your web service ',
	'smw_wws_succ_created' => ' has been successfully created. Now you can use that web service in an article. You can do that manually by using the #ws parser function or via the GUI for using a web service. This GUI is available via the semantic toolbar.',
	'smw_wws_succ_created-2' => 'Your web service will from now on be available in ',
	'smw_wws_succ_created-3' => 'the list of available WebServices.', 
