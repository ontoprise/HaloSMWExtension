...
class DIWebService {
	...
	private $mWSClient;		  //IDIWebServiceClient: the web service client provides
	...
	public static function newFromName($name) {
		...
		$id = $t->getArticleID();
		return ($id < 0) ? null : difGetWSStore()->getWS($id);
	}
	...
	public function call($parameterSetId, $resultParts) {
		...
		$specParameters = difGetWSStore()->getParameters($parameterSetId);
		
		$this->initializeCallParameters($specParameters);

		$cacheResult = difGetWSStore()->getResultFromCache($this->mArticleID, $parameterSetId);
		$response = null;
		...
		if($cacheResult != null){
			if(($this->mDisplayPolicy == 0) ||
				...
				difGetWSStore()->updateCacheLastAccess($this->mArticleID, $parameterSetId);
			}
		}
		...
		if($this->getConfirmationStatus() == "false"){
			...
			difGetWSStore()->setWWSDConfirmationStatus($this->mArticleID, "once");
		}
	...
	private function evaluateAdditionalPathAttribute($rdef, $alias, $value){
		...
		if($xpath != null){
			...
			foreach($value as $v){
				...
				$xpathProcessor = new DIXPathProcessor($v);
	...
	private function getResults($response, $resultDef, $alias) {
		...
		$xpathProcessor = new DIXPathProcessor($response);
	...
	public static function flattenParam($name, $type, $wsClient, &$typePath=null) {
		...
		foreach ($tp as $var => $type) {
			...
			if ($wsClient->isCustomType($type)) {
				...
				$typePath[] = $type;
				$names = self::flattenParam($fname, $type, $wsClient, $typePath);
	...
	public function removeFromDB() {
		difGetWSStore()->removeWS($this->getArticleID());
	}
	...
	/**
	 * Returns an instance of IDIWebServiceClient
	 *
	 * @return IDIWebServiceClient
	 *
	 *
	 */
	...
	public function checkParameterArrayIndexes($paths, &$msg){
		...
		foreach($paths as $path){
			...
			foreach($pathSteps as $pathStep){
				if(!(self::getReturnPartBracketValue($pathStep) === false)){
					if(self::getReturnPartBracketValue($pathStep) !== true){
						$index = self::getReturnPartBracketValue($pathStep);
						$tAPath = $aPath.".".self::getReturnPartPathStep($pathStep);
