...
class CEDBHelper {
	...
	public static function setupTable($table, $fields, $db, $verbose, $primaryKeys = "") {
		global $wgDBname;
		CEDBHelper::reportProgress("Setting up table $table ...\n",$verbose);
		if ($db->tableExists($table) === false) { // create new table
			...
			$sql .= ') TYPE=myisam';
			$db->query( $sql, 'CEDBHelper::setupTable' );
			CEDBHelper::reportProgress("   ... new table created\n",$verbose);
			return array();
		} else { // check table signature
			CEDBHelper::reportProgress("   ... table exists already, checking structure ...\n",$verbose);
			$res = $db->query( 'DESCRIBE ' . $table, 'CEDBHelper::setupTable' );
			$curfields = array();
			$result = array();
			...
			foreach ($fields as $name => $type) {
				if ( !array_key_exists($name,$curfields) ) {
					CEDBHelper::reportProgress("   ... creating column $name ... ",$verbose);
					$db->query("ALTER TABLE $table ADD `$name` $type $position", 'CEDBHelper::setupTable');
					$result[$name] = 'new';
					CEDBHelper::reportProgress("done \n",$verbose);
				} elseif ($curfields[$name] != $type && stripos("auto_increment", $type) == -1) {
					CEDBHelper::reportProgress("   ... changing type of column $name from '$curfields[$name]' to '$type' ... ",$verbose);
					$db->query("ALTER TABLE $table CHANGE `$name` `$name` $type $position", 'CEDBHelper::setupTable');
					$result[$name] = 'up';
					$curfields[$name] = false;
					CEDBHelper::reportProgress("done.\n",$verbose);
				} else {
					CEDBHelper::reportProgress("   ... column $name is fine\n",$verbose);
					$curfields[$name] = false;
				}
				$position = "AFTER $name";
			}
			foreach ($curfields as $name => $value) {
				if ($value !== false) { // not encountered yet --> delete
					CEDBHelper::reportProgress("   ... deleting obsolete column $name ... ",$verbose);
					$db->query("ALTER TABLE $table DROP COLUMN `$name`", 'CEDBHelper::setupTable');
					$result[$name] = 'del';
					CEDBHelper::reportProgress("done.\n",$verbose);
				}
			}
			CEDBHelper::reportProgress("   ... table $table set up successfully.\n",$verbose);
			return $result;
		}
	}
	...
	public static function getSQLOptionsAsString($requestoptions, $valuecol = NULL) {
		$options = CEDBHelper::getSQLOptions($requestoptions,$valuecol);
