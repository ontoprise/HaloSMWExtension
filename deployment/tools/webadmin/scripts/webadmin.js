...
$(function() {
	...
	$(document).ready(function(e) { 
		...
		$('#df_createBundle').click(function() { 
			var selectedBundle = $('#df_bundlename').val();
			if (!selectedBundle) {
				selectedBundle = $("#df_bundles_list option:selected").text();	
			}
			if (selectedBundle == '') {
				alert(dfgWebAdminLanguage.getMessage('df_webadmin_emptybundlename'));
				return;
			}
			var url = wgServer+wgScriptPath+"/deployment/tools/webadmin/index.php?rs=createBundle&rsargs[]="+encodeURIComponent(selectedBundle);
			$('#df_bundleexport_progress_indicator').show();
			$('#df_createBundle').attr('disabled', true);
			$.ajax( { url : url, dataType:"json", complete : function(xhr, status) {
					$('#df_bundleexport_progress_indicator').hide();
					$('#df_createBundle').attr('disabled', false);
					var result = $.parseJSON(xhr.responseText);
					if (result.returnCode != 0) {
						$('#df_contentbundle_error').html(dfgWebAdminLanguage.getMessage('df_webadmin_contentbundle_error')+'<a target="_blank" href="'
								+wgServer+wgScriptPath+'/deployment/tools/webadmin/index.php?rs=readLog&rsargs[]='+result.outputFile+'&rsargs[]=text">log</a>.');
						return;
					} else {
						// ok, remove error hint if any
						$('#df_contentbundle_error').html('');
					}
					window.location.href = wgServer+wgScriptPath+"/deployment/tools/webadmin/index.php?tab=8"; 
				}
			});
		});
	});
