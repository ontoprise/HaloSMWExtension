...
class TestOntologyMerger extends PHPUnit_Framework_TestCase {
	...
	function testPropertyPrefixMappings() {
		$om = new OntologyMerger("PR_", array("Knows"));
	    $actual = $om->transformOntologyElements("[[Knows::Kai]] [[Name::Kai]]");
	    $this->assertEquals("[[PR_Knows:::PR_Kai]] [[PR_Name::Kai]]", $actual);
	}

	function testPropertyPrefixMappings2() {
		$om = new OntologyMerger("PR_", array(), array("Has domain and range"=> array("Type:Page","Type:Page")));
		$actual = $om->transformOntologyElements("[[Has domain and range::Category:Test]]");
		$this->assertEquals("[[Has domain and range::Category:PR_Test]]", $actual);
	}
	
    function testPropertyPrefixMappings3() {
        $om = new OntologyMerger("PR_", array(), array("Has domain and range"=> array("Type:Page","Type:Page")));
        $actual = $om->transformOntologyElements("[[Has domain and range :: Category:Test1; Category:Test2]]");
        $this->assertEquals("[[Has domain and range::Category:PR_Test1; Category:PR_Test2]]", $actual);
    }
    
    function testCategoryPrefix() {
        $om = new OntologyMerger("PR_", array(), array("Has domain and range"=> array("Type:Page","Type:Page")));
        $actual = $om->transformOntologyElements("[[Category:Test1]]");
        echo $actual;
    ...
    function testCategoryPrefix2() {
        $om = new OntologyMerger("PR_", array(), array("Has domain and range"=> array("Type:Page","Type:Page")));
        $actual = $om->transformOntologyElements("[[ category : Test2]]");
        echo $actual;
