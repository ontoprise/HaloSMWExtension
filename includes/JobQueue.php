...
abstract class Job {
	 ...
	 * $command has to match the job's classname!
	 */
	static function factory( $command, $title, $params = false, $id = 0 ) {
		...
		/**
		 * autoloader for customjobs.
		 * cannot be replaced with a hook, because hooks cannot return objects.
		 */
		try {
			...
			throw new MWException("Invalid job command \"$command\". Could not load class " . $jobfilename . " ");
		}
	}
